...
def _to_xml(req, resp, indent):
    ...
    # Sanitize FT tree: ensure children are strings or FT nodes
    def _sanitize(o):
        # If FT, sanitize its children in place
        if isinstance(o, FT):
            new_children = []
            for c in o.children:
                if isinstance(c, (FT, str, bytes)):
                    new_children.append(c)
                elif isinstance(c, (list, tuple)):
                    # sanitize sequences by converting non-FT elements to str
                    seq = []
                    for e in c:
                        if isinstance(e, FT):
                            _sanitize(e)
                            ...
                            try:
                                ...
                                seq.append('')
                    new_children.append(type(c)(seq))
                else:
                    try:
                        new_children.append(str(c))
                    except Exception:
                        new_children.append('')
            o.children = new_children
            for c in o.children:
                _sanitize(c)
        # If sequence (tuple/list), sanitize elements
            ...
            for c in o:
                if isinstance(c, FT):
                    ...
                    new_seq.append(type(c)([str(x) if not isinstance(x, (FT,str,bytes)) else x for x in c]))
                else:
                    try:
                        new_seq.append(str(c))
                    except Exception:
                        new_seq.append('')
            return type(o)(new_seq)

    # Sanitize response and any injected fragments
    _sanitize(resp)
    if hasattr(req, 'injects') and req.injects:
        req.injects = [i if isinstance(i, FT) else (i if isinstance(i, str) else str(i)) for i in req.injects]
        for i in req.injects:
            _sanitize(i)

    # Debug: print types in resp and injects if a problematic type remains
    try:
        # Build flat list of top-level elements to be passed to to_xml
        top = resp if isinstance(resp, (list,tuple)) else (resp,)
        bad = []
        for i, e in enumerate(top):
            if not isinstance(e, (FT, str, bytes)):
                bad.append((i, type(e), repr(e)[:200]))
        if bad:
            print('[fasthtml._to_xml] Found non-FT/non-str top-level elements:', bad)
            print('[fasthtml._to_xml] req.injects:', [(type(x), repr(x)[:200]) for x in getattr(req, 'injects', [])])
    except Exception:
        pass
    # Deep inspect for True values
    try:
        paths = []
        def _deep(o, path='root'):
            if isinstance(o, bool) and o is True:
                paths.append(path)
                return
            if isinstance(o, FT):
                for i,c in enumerate(o.children): _deep(c, f"{path}/{o.tag}[{i}]")
            elif isinstance(o, (list,tuple)):
                for i,c in enumerate(o): _deep(c, f"{path}/tuple[{i}]")
        _deep(resp)
        for i, inj in enumerate(getattr(req, 'injects', []) or []):
            _deep(inj, f"injects[{i}]")
        if paths:
            print('[fasthtml._to_xml] Found True at paths:', paths)
    except Exception:
        pass
