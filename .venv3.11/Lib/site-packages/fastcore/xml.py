...
def to_xml(*elms, lvl=0, indent=True, do_escape=True):
    ...
    def _f(elm):
        ...
        if isinstance(elm, bytes): return elm.decode('utf-8')
        # Ensure we always return a string: convert non-string truthy values to str,
        # and falsy/None values to empty string.
        if elm is None: return ''
        if isinstance(elm, str): return elm
        return str(elm) if elm else ''
    return Safe('\n'.join(map(_f, elms)))
